# Adapted from OpenAI's circuit_sparsity codebase (https://github.com/openai/circuit_sparsity),
# Copyright 2025 OpenAI, licensed under the Apache License, Version 2.0. See LICENSE and NOTICE.
"""Minimal kernel implementations used by :mod:`runtime.inference.gpt`.
...
of the API exercised by :class:`~runtime.inference.gpt.GPT`.
"""
...
def topk(x, k, abs: bool = False, exact: bool = True, dim=None):
    ...
    if exact:
        ...
        raise NotImplementedError("approximate topk is not available in the supplementary build")
    ...
    if dim is None:
        ...
        k //= x.shape[1 - dim]
        assert len(x.shape) == 2, "expected a 2D tensor"
