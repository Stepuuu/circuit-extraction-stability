"""Factorized query/key variant of the circuit-analysis core.

Same learned-mask machinery as factorized_graph_core, but query-side and
key-side attention support are kept as separate attn_q / attn_k node families
instead of one merged attn_qk node type.
"""
...
from factorized_graph_core import compute_gao_mean_activations, _apply_feature_mask


RESID_READ_ATTN_KEY = "attn_resid_reads"
ATTN_Q_KEY = "attn_q_channels"
ATTN_K_KEY = "attn_k_channels"
...
class _GaoAlignedBaseV2(nn.Module):
    ...
    def total_nodes(self) -> int:
        return (
            ...
            + 3 * self.n_layer * self.n_head * self.d_head
    ...
    def forward(self, idx: torch.Tensor, targets: torch.Tensor = None):
        ...
        for layer_idx, block in enumerate(self.model.transformer.h):
            ...
            q_mask = masks[ATTN_Q_KEY][layer_idx].view(1, self.n_head, 1, self.d_head)
            q_mean = self.q_means.get(layer_idx)
            if q_mean is not None:
                q_mean = q_mean.view(1, self.n_head, 1, self.d_head).to(device).expand_as(q)
            q = _apply_feature_mask(q, q_mask, q_mean)

            k_mask = masks[ATTN_K_KEY][layer_idx].view(1, self.n_head, 1, self.d_head)
            ...
            k = _apply_feature_mask(k, k_mask, k_mean)
            ...
            y = F.scaled_dot_product_attention(q, k, v, dropout_p=0.0, is_causal=True, scale=1.0 / (self.d_head ** 0.5))
...
class GaoAlignedMaskedModelV2(_GaoAlignedBaseV2):
    def __init__(self, model, temperature: float = 2 / 3, stretch_lo: float = -0.1, stretch_hi: float = 1.1):
        ...
        self.attn_q_masks = HardConcreteMask(self.n_layer * self.n_head * self.d_head, temperature, stretch_lo, stretch_hi)
        self.attn_k_masks = HardConcreteMask(self.n_layer * self.n_head * self.d_head, temperature, stretch_lo, stretch_hi)
    ...
    def _mask_tensors(self, device: torch.device) -> Dict[str, torch.Tensor]:
        return {
            ...
            ATTN_Q_KEY: self.attn_q_masks().view(self.n_layer, self.n_head, self.d_head).to(device),
            ATTN_K_KEY: self.attn_k_masks().view(self.n_layer, self.n_head, self.d_head).to(device),
    ...
    def l0_loss(self) -> torch.Tensor:
        return (
            ...
            + self.attn_q_masks.l0_loss()
            + self.attn_k_masks.l0_loss()
    ...
    def get_circuit(self, threshold: float = 0.5) -> dict:
        circuit = {
            ...
            "node_schema": "gao_aligned_v2",
        }
        attn_read_vals = self.attn_read_masks.forward(deterministic=True).view(self.n_layer, self.d_model)
        attn_q_vals = self.attn_q_masks.forward(deterministic=True).view(self.n_layer, self.n_head, self.d_head)
        attn_k_vals = self.attn_k_masks.forward(deterministic=True).view(self.n_layer, self.n_head, self.d_head)
        ...
        for l in range(self.n_layer):
            ...
            circuit["attn_q_mask_values"][str(l)] = attn_q_vals[l].reshape(-1).tolist()
            circuit["attn_k_mask_values"][str(l)] = attn_k_vals[l].reshape(-1).tolist()
            ...
            for h in range(self.n_head):
                for c in range(self.d_head):
                    if attn_q_vals[l, h, c] > threshold:
                        circuit[ATTN_Q_KEY].append((l, h, c))
                    if attn_k_vals[l, h, c] > threshold:
                        circuit[ATTN_K_KEY].append((l, h, c))
        ...
        circuit["num_attn_q_channels"] = len(circuit[ATTN_Q_KEY])
        circuit["num_attn_k_channels"] = len(circuit[ATTN_K_KEY])
        ...
        circuit["total_nodes"] = (
            circuit["num_attn_resid_reads"] + circuit["num_attn_q_channels"] + circuit["num_attn_k_channels"] +
            circuit["num_attn_v_channels"] + circuit["num_attn_resid_writes"] + circuit["num_mlp_resid_reads"] +
            circuit["num_mlp_neurons"] + circuit["num_mlp_resid_writes"]
        )
...
class GaoAlignedAblationModelV2(_GaoAlignedBaseV2):
    def __init__(self, model):
        super().__init__(model)
        self.attn_read_keep = torch.ones(self.n_layer, self.d_model, dtype=torch.bool)
        self.attn_q_keep = torch.ones(self.n_layer, self.n_head, self.d_head, dtype=torch.bool)
        self.attn_k_keep = torch.ones(self.n_layer, self.n_head, self.d_head, dtype=torch.bool)
    ...
    def _mask_tensors(self, device: torch.device) -> Dict[str, torch.Tensor]:
        return {
            ...
            ATTN_Q_KEY: self.attn_q_keep.float().to(device),
            ATTN_K_KEY: self.attn_k_keep.float().to(device),
    ...
    def reset_masks(self) -> None:
        ...
        self.attn_q_keep.fill_(True)
        self.attn_k_keep.fill_(True)
    ...
    def _set_nodes(self, circuit: dict, value: bool) -> None:
        ...
        for l, h, c in circuit.get(ATTN_Q_KEY, []):
            self.attn_q_keep[l, h, c] = value
        for l, h, c in circuit.get(ATTN_K_KEY, []):
            self.attn_k_keep[l, h, c] = value
    ...
    def set_keep_circuit(self, circuit: dict) -> None:
        ...
        self.attn_q_keep.fill_(False)
        self.attn_k_keep.fill_(False)
...
def annotate_gao_edge_stats_v2(model, circuit: dict) -> dict:
    ...
    if not hasattr(model, "_gao_total_nonzero_edges_v2"):
        total = 0
        for block in model.transformer.h:
            w_attn = block.attn.c_attn.weight.data
            total += torch.count_nonzero(w_attn[:nhd]).item()
            total += torch.count_nonzero(w_attn[nhd:2 * nhd]).item()
            total += torch.count_nonzero(w_attn[2 * nhd:3 * nhd]).item()
        ...
        model._gao_total_nonzero_edges_v2 = total

    selected = {
        RESID_READ_ATTN_KEY: {l: torch.zeros(d_model, dtype=torch.bool) for l in range(n_layer)},
        ATTN_Q_KEY: {l: torch.zeros(n_head, d_head, dtype=torch.bool) for l in range(n_layer)},
        ATTN_K_KEY: {l: torch.zeros(n_head, d_head, dtype=torch.bool) for l in range(n_layer)},
    ...
    for l, h, c in circuit.get(ATTN_Q_KEY, []):
        selected[ATTN_Q_KEY][l][h, c] = True
    for l, h, c in circuit.get(ATTN_K_KEY, []):
        selected[ATTN_K_KEY][l][h, c] = True
    for l, h, c in circuit.get(ATTN_V_KEY, []):
    ...
    for l, block in enumerate(model.transformer.h):
        ...
        q_sel = selected[ATTN_Q_KEY][l].reshape(-1)
        k_sel = selected[ATTN_K_KEY][l].reshape(-1)
        ...
        if attn_read_sel.any() and q_sel.any():
            breakdown["attn_read_to_q"] += torch.count_nonzero(q_w[q_sel][:, attn_read_sel]).item()
        if attn_read_sel.any() and k_sel.any():
            breakdown["attn_read_to_k"] += torch.count_nonzero(k_w[k_sel][:, attn_read_sel]).item()
    ...
    circuit["total_possible_edges"] = model._gao_total_nonzero_edges_v2
    circuit["circuit_edge_fraction"] = edge_count / max(model._gao_total_nonzero_edges_v2, 1)
